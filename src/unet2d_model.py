...
class Generator(nn.Module):
    def __init__(self, in_channels=1, out_channels=3, features=64):
        ...
        self.near_sqrt = int(np.sqrt(c.NEAR_SQUARE))
        ...
        self.up1 = Block(features * 8, features * 8, down=False, act="relu", use_dropout=True, kernel=4, stride=1, pad=1)
        self.up2 = Block(features * 8 + features * 8, features * 8, down=False, act="relu", use_dropout=True, kernel=3, stride=2, pad=1)
        self.up3 = Block(features * 8 + features * 8, features * 8, down=False, act="relu", use_dropout=True, kernel=3, stride=2, pad=1)
        self.up4 = Block(features * 8 + features * 8, features * 4, down=False, act="relu", use_dropout=False, kernel=4, stride=2, pad=1)
        self.up5 = Block(features * 4 + features * 4, features * 2, down=False, act="relu", use_dropout=False, kernel=3, stride=2, pad=1)
        self.up6 = Block(features * 2 + features * 2, features, down=False, act="relu", use_dropout=False, kernel=4, stride=2, pad=1)
        self.final_up = nn.Sequential(
            nn.ConvTranspose2d(features + features, 1, kernel_size=4, stride=2, padding=0),
            nn.Tanh(),
        )
    ...
    def forward(self, x):
        ...
        u5 = self.up5(torch.cat([u4, d3_ip], 1)) # 256x96x96
        #print("u5:", u5.shape, "d2:", d2.shape)
        d2_ip = func.interpolate(d2, u5.shape[2:], mode='bilinear')
        u6 = self.up6(torch.cat([u5, d2_ip], 1)) # 128x96x96     
        #print("u6:", u6.shape, "d1:", d1.shape)
        d1_ip = func.interpolate(d1, u6.shape[2:], mode='bilinear')
        u7 = self.final_up(torch.cat([u6, d1_ip], 1)) # 106x96x96
        #print("u7:", u7.shape)
        shuffle = func.pixel_unshuffle(u7, downscale_factor=self.near_sqrt)
        crop = shuffle[:, :self.out_channels]
        return crop
